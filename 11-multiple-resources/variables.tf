variable "subnet_config" {
  ...
  validation {
    ...
    error_message = "At least one of the provided CIDR blocks is invalid"
  }
}
...
variable "ec2_instance_config_map" {
  ...
  # Ensure that the subnet_name is valid
  validation {
    condition = alltrue(
      [
        for config in values(var.ec2_instance_config_map) : contains(keys(var.subnet_config), config.subnet_name)
      ]
    )
